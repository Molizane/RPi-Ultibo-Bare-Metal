...
{                                                                              }
{ An LED and a 330 ohm resistor connected between GPIO Pin 21 and Ground        }
...
begin
 ...
 {Now set GPIO pin 0 to Pull None}
 GPIOPullSelect(GPIO_PIN_21,GPIO_PULL_NONE);
 
 {And make GPIO pin 0 an Output so we can turn the LED on or off}
 GPIOFunctionSelect(GPIO_PIN_21,GPIO_FUNCTION_OUT);
 
 {Finally set the value of GPIO pin 0 to Low so the LED will be off}
 ...
 while True do
  begin
   ...
   ConsoleWindowWriteLn(WindowHandle,'Turning on the LED (GPIO 21)');

   {Turn on the LED}
   GPIOOutputSet(GPIO_PIN_21,GPIO_LEVEL_HIGH);

   {Wait for a while so that things happen at a speed we can see, 1000 milliseconds,
    or half a second, should be enough.}
   ThreadSleep(1000);

   {Write to the console}
   ConsoleWindowWriteLn(WindowHandle,'Turning off the LED (GPIO 21)');
